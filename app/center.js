...
exports.centerMgr = {
  ...
  getCenters : function(limit,cb){
    mysqlMgr.connect(function (conn) {
      conn.query('SELECT  *  FROM `centers` WHERE `status` = 1 AND `idcenter` > 0 AND `constituency_idconstituency` <> 0 AND `mahalla_idmahalla` <> 0  AND `subconstituency_idsubconstituency` <> 0 AND `village_idvillage` <>  0 limit ?,10; SELECT COUNT(*) as cnt FROM `centers`  WHERE `status` = 1 AND `constituency_idconstituency` <> 0 AND `mahalla_idmahalla` <> 0  AND `subconstituency_idsubconstituency` <> 0 AND `village_idvillage` <>  0   ;',limit,  function(err, result) {
        conn.release();
        if(err) {
          util.log(err);
        } else {
  ...
  getCentersOffice : function(limit,id,cb){
    mysqlMgr.connect(function (conn) {
      conn.query('SELECT  *  FROM `centers` WHERE `status` = 1 AND `idcenter` > 0 AND `constituency_idconstituency` <> 0 AND `mahalla_idmahalla` <> 0  AND `subconstituency_idsubconstituency` <> 0 AND `village_idvillage` <>  0  AND `office_idoffice`= ? limit ?,10; SELECT COUNT(*) as cnt FROM `centers`  WHERE `status` = 1 AND `constituency_idconstituency` <> 0 AND `mahalla_idmahalla` <> 0  AND `subconstituency_idsubconstituency` <> 0 AND `village_idvillage` <>  0 AND `office_idoffice`= ?; ',[id,limit,id] ,function(err, result) {
        conn.release();
        if(err) {
          util.log(err);
        } else {
  ...
  getCentersS : function(id,cid,cb){
    mysqlMgr.connect(function (conn) {
      conn.query('SELECT  *  FROM `centers` WHERE `status` = 1 AND `constituency_idconstituency` <> 0 AND `mahalla_idmahalla` <> 0  AND `subconstituency_idsubconstituency` <> 0 AND `village_idvillage` <>  0 AND `office_idoffice`= ?  AND`subconstituency_idsubconstituency`= ?',[id,cid],  function(err, result) {
        conn.release();
        if(err) {
          util.log(err);
        } else {
  ...
  searchByCenterId : function(id,level,ido,cb){
    ...
    mysqlMgr.connect(function (conn) {
      if(level<2){
        ...
        conn.query('SELECT  *  FROM `centers` WHERE `status` = 1 AND `constituency_idconstituency` <> 0 AND `mahalla_idmahalla` <> 0  AND `subconstituency_idsubconstituency` <> 0 AND `village_idvillage` <>  0  AND `office_idoffice`=? AND `center_id` LIKE ? ',[ido,id],  function(err, result) {
          conn.release();
          if(err) {
            util.log(err);
          } else {
  ...
  iscenter: function(cid,id,cb){
    mysqlMgr.connect(function (conn) {
      conn.query('SELECT `idcenter`  FROM `centers` WHERE `status` = 1 AND `constituency_idconstituency` <> 0 AND `mahalla_idmahalla` <> 0  AND `subconstituency_idsubconstituency` <> 0 AND `village_idvillage` <>  0  AND `center_id`=? AND `office_idoffice`= ?',[cid,id],  function(err, result) {
        conn.release();
        if(err) {
          util.log(err);
        } else {
